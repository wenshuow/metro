...
asymean = 1/math.sqrt(2*math.pi)-1/2 # mean, to be adjusted
variance = 1.5-(1/math.sqrt(2*math.pi)-1/2)**2 # variance, to be adjusted
...
def p_marginal_trans_log(rhos, p, j, xj, xjm1):
    if j>p:
        return("error!")
    if j==1:
        if xj*math.sqrt(variance)+asymean>=0:
            return(dnormlog(xj*math.sqrt(variance)+asymean)+0.5*log(variance)+log(2))
        else:
            return(dexplog(-(xj*math.sqrt(variance)+asymean)))+0.5*log(variance)
    j = j - 1
    if (xj-rhos[j-1]*xjm1)*math.sqrt(variance)/math.sqrt(1-rhos[j-1]**2)+asymean>=0:
        return(dnormlog((xj-rhos[j-1]*xjm1)*math.sqrt(variance)/math.sqrt(1-rhos[j-1]**2)+asymean)+0.5*log(variance)-0.5*log(1-rhos[j-1]**2)+log(2))
    else:
        return(dexplog(-((xj-rhos[j-1]*xjm1)*math.sqrt(variance)/math.sqrt(1-rhos[j-1]**2)+asymean))+0.5*log(variance)-0.5*log(1-rhos[j-1]**2))

def p_marginal_log(rhos, p, x):
  p_dim = len(x)
  res = p_marginal_trans_log(rhos, p, 1, x[0], 0)
  if p_dim==1:
    return(res)
  for j in list(range(2,p_dim+1)):
    res = res + p_marginal_trans_log(rhos, p, j, x[j-1], x[j-2])
  return(res)

def SCEP_MH_MC(x_obs, gamma, x_grid, rhos):
  p = len(x_obs)
  ...
  for k in range(2*midtry-2):
      parallel_marg_density_log[k,0] = marg_density_log + p_marginal_trans_log(rhos, p, 1,ws[k,0],0) + p_marginal_trans_log(rhos, p, 2,x_obs[1],ws[k,0]) - p_marginal_trans_log(rhos, p, 1,x_obs[0],0) - p_marginal_trans_log(rhos, p, 2,x_obs[1],x_obs[0])
  probs = [0]*(2*midtry-2)
  for k in range(2*midtry-2):
  ...
  for k in range(2*midtry-2):
      refs_parallel_marg_density_log[k,0] = marg_density_log + p_marginal_trans_log(rhos, p, 1,refws[k,0],0) + p_marginal_trans_log(rhos, p, 2,x_obs[1],refws[k,0]) - p_marginal_trans_log(rhos, p, 1,x_obs[0],0) - p_marginal_trans_log(rhos, p, 2,x_obs[1],x_obs[0])
  ...
  for j in range(1,p):
    ...
    for k in range(2*midtry-2):
      ...
      if j+1==p:
        parallel_marg_density_log[k,j] = marg_density_log + p_marginal_trans_log(rhos, p, p,ws[k,j],x_obs[j-1]) - p_marginal_trans_log(rhos, p, p,x_obs[j],x_obs[j-1])
      if j+1>1 and j+1<p:
        parallel_marg_density_log[k,j] = marg_density_log + p_marginal_trans_log(rhos, p, j+1,ws[k,j],x_obs[j-1]) + p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],ws[k,j]) - p_marginal_trans_log(rhos, p, j+1,x_obs[j],x_obs[j-1]) - p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],x_obs[j])
    for k in range(2*midtry-2):
      ...
      for k2 in range(2*midtry-2):
        ...
        probs[k2] = parallel_cond_density_log[k2,j-1] + parallel_marg_density_log[k2,j-1] + p_marginal_trans_log(rhos, p, j+1,ws[k,j],ws[k2,j-1]) - p_marginal_trans_log(rhos, p, j+1,x_obs[j],ws[k2,j-1])
        if j+2<=p:
          probs[k2] = probs[k2] + p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],ws[k,j]) - p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],x_obs[j])
      oriprobs = probs
      probs = [x-max(probs) for x in probs]
      probs = [math.exp(x) for x in probs]
      probs = [x/sum(probs) for x in probs]
      ...
      for k2 in range(2*midtry-2):
        ...
        refprobs[k2] = refs_parallel_cond_density_log[k2,j-1] + refs_parallel_marg_density_log[k2,j-1] + p_marginal_trans_log(rhos, p, j+1,ws[k,j],refws[k2,j-1]) - p_marginal_trans_log(rhos, p, j+1,x_obs[j],refws[k2,j-1])
        if j+2<=p:
          refprobs[k2] = refprobs[k2] + p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],ws[k,j]) - p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],x_obs[j])
      remove = max(refprobs+oriprobs)
      refprobs = [x-remove for x in refprobs]
      oriprobs = [x-remove for x in oriprobs]
      acc_rg = min(1,sum([math.exp(x) for x in oriprobs])/sum([math.exp(x) for x in refprobs]))
    ...
    for k in range(2*midtry-2):
      ...
      if j+1==p:
        refs_parallel_marg_density_log[k,j] = marg_density_log + p_marginal_trans_log(rhos, p, p,refws[k,j],x_obs[j-1]) - p_marginal_trans_log(rhos, p, p,x_obs[j],x_obs[j-1])
      if j+1>1 and j+1<p:
        refs_parallel_marg_density_log[k,j] = marg_density_log + p_marginal_trans_log(rhos, p, j+1,refws[k,j],x_obs[j-1]) + p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],refws[k,j]) - p_marginal_trans_log(rhos, p, j+1,x_obs[j],x_obs[j-1]) - p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],x_obs[j])
    for k in range(2*midtry-2):
      # if started from Wrj,k instead of Xj
      probs = [0]*(2*midtry-2)
      for k2 in range(2*midtry-2):
        # probs[k2] is X1; ...; Wj-1,k2; Wrj,k; ...; Xp; tX1; ...; tXj-2, densities
        probs[k2] = parallel_cond_density_log[k2,j-1] + parallel_marg_density_log[k2,j-1] + p_marginal_trans_log(rhos, p, j+1,refws[k,j],ws[k2,j-1]) - p_marginal_trans_log(rhos, p, j+1,x_obs[j],ws[k2,j-1])
        if j+2<=p:
          probs[k2] = probs[k2] + p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],refws[k,j]) - p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],x_obs[j])
      oriprobs = probs
      probs = [x-max(probs) for x in probs]
      probs = [math.exp(x) for x in probs]
      probs = [x/sum(probs) for x in probs]
      ...
      for k2 in range(2*midtry-2):
        ...
        refprobs[k2] = refs_parallel_cond_density_log[k2,j-1] + refs_parallel_marg_density_log[k2,j-1] + p_marginal_trans_log(rhos, p, j+1,refws[k,j],refws[k2,j-1]) - p_marginal_trans_log(rhos, p, j+1,x_obs[j],refws[k2,j-1])
        if j+2<=p:
          refprobs[k2] = refprobs[k2] + p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],refws[k,j]) - p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],x_obs[j])
      remove = max(refprobs+oriprobs)
      refprobs = [x-remove for x in refprobs]
      oriprobs = [x-remove for x in oriprobs]
      acc_rg = min(1,sum([math.exp(x) for x in oriprobs])/sum([math.exp(x) for x in refprobs]))
...
def get_cov_guided(p, rhos):
  ...
  for i in range(p-1):
      for j in range(i+1,p):
          for k in range(i,j):
              cormatrix[i,j] = cormatrix[i,j]*rhos[k]
          cormatrix[j,i] = cormatrix[i,j]
  d_sdp = sdp_solver(p, rhos) # use SDP for diag(s); other options include ASDP, equicorrelated, etc.
  ...
  return [prop_mat, cond_means_coeff, cond_vars, d_sdp]


def SCEP_MH_MC_COV(x_obs, gamma, mu_vector, solve_out, rhos):
  p = len(x_obs)
  cond_coeff, cond_means_coeff, cond_vars, d_sdp = solve_out
  ...
  for i in range(p-1):
      for j in range(i+1,p):
          for k in range(i,j):
              cormatrix[i,j] = cormatrix[i,j]*rhos[k]
          cormatrix[j,i] = cormatrix[i,j]
  Cov_matrix = cormatrix
  Cov_matrix_off = np.matrix.copy(Cov_matrix)
  for i in range(p):
      Cov_matrix_off[i,i] = Cov_matrix[i,i] - d_sdp[i]
  ...
  for j in range(p):
    if j==0:
        parallel_marg_density_log[j] = marg_density_log + p_marginal_trans_log(rhos, p, 1,ws[0],0) + p_marginal_trans_log(rhos, p, 2,x_obs[1],ws[0]) - p_marginal_trans_log(rhos, p, 1,x_obs[0],0) - p_marginal_trans_log(rhos, p, 2,x_obs[1],x_obs[0])
    if j==p-1:
        parallel_marg_density_log[j] = marg_density_log + p_marginal_trans_log(rhos, p, p,ws[p-1],x_obs[p-2]) - p_marginal_trans_log(rhos, p, p,x_obs[p-1],x_obs[p-2])
    if j>0 and j<p-1:
        parallel_marg_density_log[j] = marg_density_log + p_marginal_trans_log(rhos, p, j+1,ws[j],x_obs[j-1]) + p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],ws[j]) - p_marginal_trans_log(rhos, p, j+1,x_obs[j],x_obs[j-1]) - p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],x_obs[j])
  cond_density_log = 0
  ...
  for j in range(p):
    ...
    if math.log(np.random.uniform())<=acc_ratio_log + math.log(gamma):
        ...
        if j+2<=p:
            ...
            alter_vec_j[j] = ws[j]
            j_acc_ratio_log = q_prop_pdf_log(j, alter_vec_j, x_obs[j]) + parallel_cond_density_log[j] + parallel_marg_density_log[j] + p_marginal_trans_log(rhos, p, j+2,ws[j+1],ws[j]) - p_marginal_trans_log(rhos, p, j+2,x_obs[j+1],ws[j])
            if j+3<=p:
                j_acc_ratio_log = j_acc_ratio_log + p_marginal_trans_log(rhos, p, j+3,x_obs[j+2],ws[j+1]) - p_marginal_trans_log(rhos, p, j+3,x_obs[j+2],x_obs[j+1])
