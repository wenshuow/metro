...
Xk = foreach(i=1:n, .combine='rbind') %do% {
...
quality_grid = ggplot(cors, aes(x = i, y = j, fill = ko_orig_corr)) +
  ...
  scale_fill_continuous(high = "#000000", low = "#56B1F7", limits = c(.1, .4)) + 
  scale_y_reverse() + 
  coord_equal() +
  labs(fill = "cor") +
  theme_bw() + 
...
print(paste0("Average time per knockoff (seconds): ", round((end - start) / n, digits = 3)))
...
quality_grid = ggplot(cors, aes(x = i, y = j, fill = ko_orig_corr)) +
  geom_tile(color = "white") +
  scale_fill_continuous(high = "#000000", low = "#56B1F7", limits = c(0, .7)) + 
  scale_y_reverse() + 
  coord_equal() +
  labs(fill = "cor") +
  theme_bw() + 
