...
We find that the marginal covariance matrices are close. The fluctions of the entries of the covariance matrix should be about $\sqrt{2 \log(p * (p - 1)) / n} = $ `r 2*sqrt(p*(p-1) / n)`, so the deviation from 0 is not larger than we expect due to random fluctuations in the empirical covariance matrices.
...
cor_plot = ggplot(cor_dat %>%
                    transform(ham_distance = as.numeric(distance), correlation = as.numeric(cor)) %>%
                    filter(ham_distance > 0) %>%
                    group_by(type), 
                  aes(x = ham_distance, y = correlation, color = type, group = interaction(ham_distance, type))) + 
  geom_boxplot() + 
